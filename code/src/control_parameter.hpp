...
static constexpr inflectionPoint inflectionpoint_s[] = {{0, 20}, {30 * 60, 80}, {60 * 60, 80}, {83 * 60, 125}, {163 * 60, 125}};
static constexpr int inflectionPointSize = sizeof(inflectionpoint_s) / sizeof(inflectionPoint);

inline const double linear(const inflectionPoint pre, const inflectionPoint next, const double etime_s) {
    return (next.temp_cels - pre.temp_cels) / (next.time_s - pre.time_s) * (etime_s - pre.time_s) + pre.temp_cels;
}

inline const double targetFunction(const double elapsed_time_s, bool& is_end) {
    is_end = false;
...
inline const double feedbackTemperature(const double T0, const double T1, const double T2, const double elasped_time_s) {
    return T1;
}
